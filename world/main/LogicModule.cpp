...
{
	auto& logicConf = SystemConfig::Me()->GetWorldConfig()->logicmodule();
...
#endif
	WorkerPool::InitInstance(WorldServer::Me()->GetMainEventLoop(), work_thread_num);
	WorkerPool::Me()->Start();
	...
	// 初始化逻辑处理者
	LogicManager::Me()->RegLogic(LOGIC_NAME(GenGUID), GenGUID::InitInstance());
